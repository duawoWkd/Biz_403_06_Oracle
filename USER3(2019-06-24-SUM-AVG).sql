...
-- ASC(오름차순정렬)는 생략 할수 있다.
SELECT * FROM tbl_score
ORDER BY sc_num ASC, sc_subject ASC;    -- ASC 원래는 있어야하나 없으면 자동으로 처리 된다.

-- DESC(내림차순정렬) : 역순정렬
-- 학번으로 내림차순 정렬하고, 그 내부에서 과목으로 다시 내림차순 정렬
SELECT * FROM tbl_score                 -- DESC 내림차순 지정한 것만 내림차순으로 된다
